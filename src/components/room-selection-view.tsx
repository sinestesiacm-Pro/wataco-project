                    ...
                    <div>
                        ...
                        <RadioGroup value={mealPlan} onValueChange={setMealPlan}>
                            <div className="flex items-center space-x-2">
                                <RadioGroupItem value="breakfast" id={`breakfast-${roomOffer.id}`} disabled={!hasBreakfast} className="border-gray-400 text-primary"/>
                                <Label htmlFor={`breakfast-${roomOffer.id}`} className="flex items-center gap-2 text-card-foreground has-[:disabled]:opacity-50">
                                   <Coffee className="h-4 w-4"/> Desayuno incluido
                                </Label>
                            </div>
                            <div className="flex items-center space-x-2">
                                <RadioGroupItem value="none" id={`none-${roomOffer.id}`} className="border-gray-400 text-primary"/>
                                <Label htmlFor={`none-${roomOffer.id}`} className="flex items-center gap-2 text-card-foreground">
                                   <Salad className="h-4 w-4"/> Solo alojamiento
                                </Label>
                            </div>
                             <div className="flex items-center space-x-2">
                                <RadioGroupItem value="all-inclusive" id={`all-${roomOffer.id}`} disabled className="border-gray-400 text-primary"/>
                                <Label htmlFor={`all-${roomOffer.id}`} className="flex items-center gap-2 text-card-foreground has-[:disabled]:opacity-50">

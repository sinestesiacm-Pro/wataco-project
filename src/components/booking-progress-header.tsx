...
const Step = ({ title, isActive, isCompleted }: { title: string; isActive: boolean; isCompleted: boolean; }) => (
    <div className="flex items-center gap-2">
        {isCompleted ? (
            ...
            <Circle className={cn("h-6 w-6", isActive ? "text-primary fill-primary/20" : "text-muted-foreground")} />
        )}
        <span className={cn("font-semibold", isActive ? "text-primary" : isCompleted ? "text-foreground" : "text-muted-foreground")}>
            {title}
        </span>
    </div>
);
...
export function BookingProgressHeader({ step, isRoundTrip, origin, destination }: BookingProgressHeaderProps) {
  ...
  return (
    <div className="bg-card p-4 rounded-2xl shadow-md border mb-8">
